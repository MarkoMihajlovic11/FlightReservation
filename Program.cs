    ...
    class Program
    {
        static async Task Main(string[] args)
        {
            try
            {
                ...
                var response = ResponseService.GetSearchResponse(cheapestFlight);

                while (response == SearchResponse.ContinueSearch)
                {
                    cheapestFlight = await flightSearchService.FindCheapestFlight(flightReservation);
                    response = ResponseService.GetSearchResponse(cheapestFlight);
                }
            }
            catch (Exception ex)
            {
